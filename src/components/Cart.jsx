      ...
      <div>
        {cartItems.map(e => {
          ...
          return <div>
            ...
            <p>total: {`${e.pack ? e.total * product[0].discountPrice : e.total * product[0].originalPrice}`}</p>
            <label htmlFor="check">Possui Embalagem?<input id="check" type="checkbox" onChange={() => alterPackage(e.id)} checked={e.pack} /></label>
          </div>
        })}
      </div>
      <div>
        <p>{`Valor Final da Compra: ${finalValue(cartItems)} `}</p>
        <Link to="/payment"><button>Finalizar Pedido</button></Link>
      </div>
    </div>
  )
}
