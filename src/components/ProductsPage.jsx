...
function renderAddButtons(id, props, setShowMessage) {
  ...
  return (
    <div>
      ...
      <button disabled={(total > 0) ? false : true} onClick={() => { addToCart(id, total); setShowMessage(() => true); setTimeout(() => { setShowMessage(false) }, 1000) }}>Adicionar ao Carrinho</button>
    </div>
  );
}
